/*
 * Project: speech_daisybell
 * Description: This Arduino project uses software-based speech synthesis to 
 *              generate the song "Daisy Bell" (Bicycle Built for Two).
 *              The song is synthesized using simple audio synthesis methods.
 *
 * Original Source: 
 *   The original code was obtained from the Google Code Archive 
 *   project "tinkerit" (link: https://storage.googleapis.com/google-code-archive-downloads/v2/code.google.com/tinkerit/speech_daisybell.pde).
 *   The author of the original code is unknown.
 *
 * Updates and Porting: 
 *   - Converted to Arduino IDE (.ino format) by Marcel Licence
 *   - Added support for modern Arduino platforms
 *   - Ensured compatibility with newer Arduino boards (e.g., Raspberry Pi Pico)
 *
 * Requirements:
 *   - Arduino IDE 1.8.x or newer
 *   - Arduino-compatible board (e.g., Raspberry Pi Pico)
 *   - A speaker connected to the designated output pin
 *
 * Usage:
 *   1. Open this file in the Arduino IDE.
 *   2. Select the appropriate board and port from the Tools menu.
 *   3. Connect a speaker to the designated pin as specified below.
 *   4. Upload the sketch and enjoy the song being synthesized.
 *
 * Authors:
 *   - Original Author: Unknown (Google Code Archive - tinkerit project)
 *   - Ported and updated by: Marcel Licence
 *
 * License:
...
/*
 * Use ML__SynthTools library, can be found on http://github.com/marcel-licence/ML__SynthTools
 */
#include <ml_types.h>
...
void setup(void)
{
    ...
    Serial.begin(115200);
    delay(1000); /* in case the device needs a moment to be recognized in the operation system */
    Serial.printf("speech daisybell port\n");
    Audio_Setup(); /* prepare RP2040 audio driver */
}
...
void loop(void)
{
    ...
    /* two buffers in case some stereo effects etc. will be added in future */
    ...
    {
        /* reading one sample after another */
        int16_t sample = speech_get_value();
        sample *= 256; /* convert int8_t to int16_t */
...
#define FORMANT__SZ 7

/* some of the enums do not work because there are defined elsewhere */
enum
{
    __SP, _DOT, _QM, _COM, _HYP, _IY, _IH, _EH, _AE, _AA,
    _AH, _AO, _UH, _AX, _IX, _ER, _UX, _OH, _RX, __LX,
    _WX, _YX, _WH, _R, __L, _W, _Y, _M, __N, __NX,
    _DX, _Q, __S, __SH, _F, _TH, __H, __X, _Z, _ZH,
...
uint8_t frameList[] PROGMEM =
{
    ...
    _AH, 25, 0, 29, __NX, 5, 0, 29, // an..
    __S, 2, 0, 32, _ER, 10, 0, 32, _RX, 3, 0, 32, // ..swer
    ...
    _AO, 10, 0, 29, __LX, 5, 0, 29, // all
    _F, 5, 0, 31, _AO, 10, 0, 31, // for
    _DH, 5, 0, 32, _AH, 10, 0, 32, // the
    __L, 5, 0, 34, _AH, 20, 0, 34, _V, 5, 0, 34, // love
    _AA, 10, 0, 36, _V, 5, 0, 36, // of
    _Y, 10, 0, 34, _UX, 80, 0, 34, // you
    _IH, 10, 0, 36, _Ta, 2, 0, 36, _Tb, 1, 0, 36, _Tc, 2, 0, 36, // It
    _W, 2, 0, 37, _OH, 10, 0, 37, __N, 1, 0, 37, _Ta, 1, 0, 37, _Tb, 1, 0, 37, _Tc, 1, 0, 37, // won't
    _Ba, 2, 0, 36, _Bb, 1, 0, 36, _Bc, 2, 0, 36, _IY, 10, 0, 36, // be
    _AH, 15, 0, 34, // a
    __S, 2, 0, 39, _Ta, 2, 0, 39, _Tb, 2, 0, 39, _Tc, 2, 0, 39, _AY, 1, 10, 39, _YX, 10, 0, 39, // sty..
    __L, 3, 0, 36, _IH, 10, 0, 36, __SH, 2, 0, 36, // ..lish
    _M, 5, 0, 34, _AE, 10, 0, 34, // ma..
    _R, 5, 0, 32, _IH, 60, 0, 32, _Ja, 2, 0, 32, _Jb, 2, 0, 32, _Jc, 2, 0, 32, // ..rriage
    _AY, 5, 10, 34, _YX, 5, 0, 34, // I
    _Ka, 2, 0, 36, _Kb, 2, 0, 36, _Kc, 2, 0, 36, _AH, 20, 0, 36, __N, 2, 0, 36, _Ta, 2, 0, 36, _Tb, 2, 0, 26, _Tc, 2, 0, 36, // can't
    _AX, 15, 0, 32, // a..
    ...
    _Y, 5, 0, 32, _UH, 10, 10, 32, __L, 5, 0, 32, // you'll
    __L, 3, 0, 36, _UH, 10, 0, 36, _Ka, 1, 0, 36, _Kb, 1, 0, 36, _Kc, 1, 0, 36, // look
    __S, 2, 0, 34, _W, 2, 0, 34, _IY, 20, 0, 34, _Ta, 2, 0, 34, _Tb, 2, 0, 34, _Tc, 2, 0, 34, // sweet
    _AX, 15, 0, 27, // a..
    _Ka, 2, 0, 32, _Kb, 2, 0, 32, _Kc, 2, 0, 32, _R, 2, 0, 32, _AA, 20, 0, 32, __S, 5, 0, 32, // ..cross
    _DH, 5, 0, 36, _AH, 10, 0, 36, // the
    __S, 2, 0, 34, _IY, 10, 0, 34, _Ta, 2, 0, 34, _Tb, 2, 0, 34, _Tc, 2, 0, 34, // seat
    _AA, 10, 0, 36, _V, 5, 0, 36, // of
    _AE, 15, 0, 37, // a
    _Ba, 2, 0, 39, _Bb, 2, 0, 39, _Bc, 2, 0, 39, _AY, 5, 5, 39, _YX, 5, 0, 39, // bi..
    __S, 5, 0, 36, _IH, 10, 0, 36, // ..cy..
    _Ka, 2, 0, 32, _Kb, 2, 0, 32, _Kc, 2, 0, 32, __L, 9, 0, 32, // ..cle
...
/*
 * use second core to drive the speech loop
 */

void setup1()
{
    /* wait a short moment to ensure the first core has finished its setup */
    delay(1000);
}
...
void speech__Loop()
{
    ...
    while (1)
    {
        ...
        formantPos = formantTable + startFormant * FORMANT__SZ;
        ...
        if (tweenFrames)
        {
            ...
            int16_t deltaPitch;
            tweenFrames--;
            formantPos = formantTable + nextFormant * FORMANT__SZ;
...
int8_t speech_get_value(void)
{
    int8_t value;
    static int8_t noise;
    int16_t phaseNoise = noise * noiseMod;
    noise += noise << 2;
    noise++; // noise' = 5*noise+1
